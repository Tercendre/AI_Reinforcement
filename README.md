...
To play:
Run the command to start the gridworld in manual mode.
...
Launch command: value agent, 100 iterations, 10 runs.
...
Note that the agent learns from the state it was just in, not the one it moved to, and "leaves learning in its wake."
...
Example: train for 2000 episodes, test for 10 on smallGrid.
...
Ce projet m’a permis de m’initier à l’apprentissage par renforcement à travers plusieurs environnements : Gridworld, Crawler et Pacman.

les fichiers dans lesquels sont implémentés les fonctions principales sont ci-dessous.
